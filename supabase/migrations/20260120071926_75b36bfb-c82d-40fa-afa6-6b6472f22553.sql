...
-- USING controls which rows can be read for update
-- WITH CHECK controls what values the new row can have
...
WITH CHECK (
  -- Allow all updates that pass the USING clause - we trust the application logic for status transitions
  true
);